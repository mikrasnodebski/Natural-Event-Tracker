...
def get_references():
    ...
    with importlib.resources.open_text('natural_event_tracker',
                                       'reference.json') as file_handle:
...
def get_events():
    ...
    events = requests.get(get_references()['find_open'], timeout=5).json()
...
def get_all_possible_event_categories():
    ...
    categories = requests.get(get_references()['find_categories'],
                              timeout=5).json()
